...
var restfulPathDropDownSalaryPeriod="/tyw_api/public/appraisal_data/period_list";
...
var getDataFn = function(){
	...
	var salaryPeriod= $("#param_salary_period").val();
	...
	$.ajax({
		...
		data:{
			...
			"appraisal_period_desc":salaryPeriod,
...
var searchAdvanceFn = function (Year,SalaryPeriod,Department,PsGroup) {
	...
	htmlParam+="<input type='hidden' class='paramEmbed' id='param_salary_period' name='param_salary_period' value='"+SalaryPeriod+"'>";
...
var dropDownListSalaryPeriod = function(year){
	...
	var html="";
	html+="<select data-toggle=\"tooltip\" title=\"Salary Period\" class=\"input form-control input-sm\" id=\"salary_period\" name=\"salary_period\" >";
	//html+="<option  selected value=''>All</option>";
	$.ajax ({
		url:restfulURL+restfulPathDropDownSalaryPeriod ,
		type:"get" ,
		dataType:"json" ,
		//data:{"year":year},
		headers:{Authorization:"Bearer "+tokenID.token},
...
$(document).ready(function() {
	...
	$("#drop_down_list_salary_period").html(dropDownListSalaryPeriod($("#year").val()));
	$("#drop_down_list_department").html(dropDownListDepartment());
	$("#drop_down_list_position_group").html(dropDownListPositionGroup());
	
	$("#year").change(function(){  
		$("#drop_down_list_salary_period").html(dropDownListSalaryPeriod($("#paramYear").val()));
	});
